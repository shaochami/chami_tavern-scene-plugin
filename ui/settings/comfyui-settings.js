...
export class ComfyUISettings {
                ...
                <div class="tsp-settings-group" style="padding: 12px; background: var(--tsp-bg-tertiary); border-radius:8px; margin-bottom:15px;">
                    ...
                    <div class="tsp-form-row">
                        ...
                        <div class="tsp-btn-group" style="flex:1; align-self: flex-end; justify-content: flex-end;">
                            <!--  更新按钮 -->
                            <button type="button" class="tsp-btn tsp-btn-primary" id="comfy-api-preset-update" title="覆盖更新当前预设">
            ...
            <div class="tsp-settings-group">
                ...
                <div class="tsp-form-group" style="margin-top: 15px; border-top: 1px solid var(--tsp-border); padding-top: 15px;">
                    ...
                    <div class="tsp-input-group">
                        ...
                        <!--  填入占位符按钮 -->
                        <button class="tsp-btn" id="workflow-autofill-btn" title="尝试自动识别并填入标准占位符">
                            <i class="fa-solid fa-magic"></i> 填入占位符
                        </button>
                        <!-- [新增结束] -->
    ...
    bindEvents(containerEl) {
        ...
        containerEl.querySelector('#comfy-api-preset-select')?.addEventListener('change', (e) => {
            ...
            if (this.ctx.getModule('settingsPanel')) {
                this.ctx.getModule('settingsPanel').isApiPresetModified.comfyui = false;
            }
        });
        containerEl.querySelector('#comfy-api-preset-update')?.addEventListener('click', async () => {
        ...
        // 防傻操作：监听API预设相关输入框变化
        const urlInput = containerEl.querySelector('#comfyui-url');
        const apiModeSelect = containerEl.querySelector('#comfyui-api-mode');
        if (urlInput) {
            urlInput.addEventListener('input', () => {
                if (this.ctx.getModule('settingsPanel')) {
                    this.ctx.getModule('settingsPanel').isApiPresetModified.comfyui = true;
                }
            });
        }
        if (apiModeSelect) {
        ...
        //  绑定自动填充按钮事件
        containerEl.querySelector('#workflow-autofill-btn')?.addEventListener('click', () => {
            this._autoFillPlaceholders();
        });
    ...
    _renderWorkflowSelect() {
        ...
        const i2iSelect = this.containerEl?.querySelector('#comfyui-i2i-workflow-select'); // 
        if (!select) return;

        const s = this._getSettings();
        select.innerHTML = '<option value="">-- 请选择工作流 --</option>';
        if (i2iSelect) i2iSelect.innerHTML = '<option value="">-- 跟随文生图或手动选择 --</option>'; // 
    ...
    /**
     *  自动填入占位符 (带防护、智能筛选、引用保护及集成节点支持)
     */
    _autoFillPlaceholders() {
        const editor = this.containerEl?.querySelector('#comfyui-workflow-editor');
        const content = editor?.value;
        ...
        try {
            ...
            this.ctx?.helpers?.showToast?.('JSON 格式错误，无法解析', 'error');
            return;
        }

        // --- 防护检查 1：防止覆盖已有配置 ---
        const placeholdersToCheck = [
            "种子", "宽度", "高度", "步数", "CFG", "采样器", "调度器",
            "主模型", "正向提示词", "正面提示词", "负面提示词", "反向提示词",
            "日期", "修复模型", "放大模型", "放大倍率", "修复步数"
        ];

        // 简单暴力检查字符串是否包含已有的占位符
        const jsonString = JSON.stringify(wf);
        const existingTags = placeholdersToCheck.filter(tag => jsonString.includes(`"${tag}"`));

        if (existingTags.length > 0) {
            alert(`⚠️ 操作取消：检测到工作流中已经包含以下占位符：\n${existingTags.join(', ')}\n\n为了防止覆盖您的人工修改，请手动编辑或先还原为原始数值。`);
            return;
        }

        // --- 收集候选 ---
        const changes = []; // 待执行的修改列表

        // 提示词专门处理：收集所有可能的节点，最后只选ID最大的一个
        ...
        // 遍历所有节点
        for (const [idStr, node] of Object.entries(wf)) {
            const id = parseInt(idStr);
            const type = node.class_type;
            const meta = node._meta || {};
            const title = (meta.title || '').toLowerCase(); // 获取小写标题方便匹配
            const inputs = node.inputs || {};

            // 辅助：添加修改计划
            const addChange = (key, placeholder) => {
                // 如果输入不存在，跳过
                if (!inputs.hasOwnProperty(key)) return;

                const val = inputs[key];

                // 1. 如果当前值已经是字符串且等于占位符，跳过
                if (val === placeholder) return;

                // 2. [核心防护] 如果值是数组 (引用了其他节点)，绝对不能修改！
                if (Array.isArray(val)) return;

                // 3. 加入修改计划
                changes.push({ id: idStr, key, val: placeholder });
            };

            // ====================== 节点类型匹配逻辑 ======================

            // 1. 基础参数 (包括 PixelKSampleUpscalerProvider 等包含 seed 的节点)
            if (inputs.hasOwnProperty('seed') || inputs.hasOwnProperty('noise_seed')) {
                const seedKey = inputs.hasOwnProperty('seed') ? 'seed' : 'noise_seed';
                // 确保只要包含种子字段，不管是 KSampler 还是 Detailer，都尝试替换
                if (type.includes('Sampler') || type.includes('Upscale') || type.includes('Detailer') || type.includes('Gene')) {
                     addChange(seedKey, '种子');
                }
            }

            // 2. KSampler / KSamplerAdvanced / PixelKSampleUpscalerProvider 通用参数
            if (type.includes('Sampler') || type.includes('Upscale')) {
                if (inputs.hasOwnProperty('steps')) addChange('steps', '步数');
                if (inputs.hasOwnProperty('cfg')) addChange('cfg', 'CFG');
                if (inputs.hasOwnProperty('sampler_name')) addChange('sampler_name', '采样器');
                if (inputs.hasOwnProperty('scheduler')) addChange('scheduler', '调度器');
            }

            // 3. 修复模型特有参数处理
            if (type === 'FaceDetailer' || type === 'FaceDetailerPipe') {
                if (inputs.hasOwnProperty('steps')) {
                    // 如果因为上面的规则 2 误加了 '步数'，这里修正为 '修复步数'
                    const idx = changes.findIndex(c => c.id === idStr && c.key === 'steps');
                    if (idx > -1) changes.splice(idx, 1);
                    addChange('steps', '修复步数');
                }
            }
            ...
            // 4.2  easy comfyLoader 等集成节点的分辨率
            if (type.includes('Loader')) {
                addChange('empty_latent_width', '宽度');
                addChange('empty_latent_height', '高度');
            }

            // 5. 模型
            // [修改] 不再判断类名，只要有 ckpt_name 且不是引用，就替换为主模型
            // 这可以兼容 easy comfyLoader, Efficiency Loader, CheckpointLoaderSimple 等所有节点
            if (inputs.hasOwnProperty('ckpt_name')) {
                addChange('ckpt_name', '主模型');
            }

            // 6. 修复/放大模型加载器
            if (type === 'UltralyticsDetectorProvider' || type === 'DownloadAndLoadUltralyticsDetector') {
                addChange('model_name', '修复模型');
            }
            if (type === 'UpscaleModelLoader') {
                addChange('model_name', '放大模型');
            }

            // 7. 放大 (IterativeLatentUpscale / UltimateSDUpscale)
            if (type === 'IterativeLatentUpscale' || type === 'UltimateSDUpscale') {
                // 如果 inputs.upscale_factor 是数字，这里会替换；如果是引用，addChange 内部会拦截
                addChange('upscale_factor', '放大倍率');
                if(inputs.upscale_by) addChange('upscale_by', '放大倍率');
            }

            // 8. 尝试识别“引用那边”的 Primitive 节点
            // 很多工作流使用 easy int, easy float, PrimitiveXXX 来控制参数
            if (type.startsWith('easy ') || type.startsWith('Primitive')) {
                const targetKey = inputs.hasOwnProperty('value') ? 'value' : null;
                if (targetKey) {
                    if (title.includes('steps') || title.includes('步数') || title.includes('迭代')) {
                        // 区分修复还是普通
                        if (title.includes('修复') || title.includes('detailer') || title.includes('inpaint')) {
                            addChange(targetKey, '修复步数');
                        } else {
                            addChange(targetKey, '步数');
                        }
                    } else if (title.includes('cfg')) {
                        addChange(targetKey, 'CFG');
                    } else if (title.includes('seed') || title.includes('种子') || title.includes('随机')) {
                        addChange(targetKey, '种子');
                    } else if (title.includes('scale') || title.includes('放大')) {
                        addChange(targetKey, '放大倍率');
                    } else if (title.includes('width') || title.includes('宽')) {
                        addChange(targetKey, '宽度');
                    } else if (title.includes('height') || title.includes('高')) {
                        addChange(targetKey, '高度');
                    }
                }
            }

            // 9. 保存日期
            if (type === 'SaveImage' || type === 'VHS_VideoSave') {
                addChange('filename_prefix', '日期');
            }

            // 10. 提示词收集 (不立即替换，先存入候选)
            if (type === 'CLIPTextEncode' || type === 'PrimitiveString' || (type.includes('Prompt') && type !== 'WeiLinPromptToString')) {
                let textKey = inputs.hasOwnProperty('text') ? 'text' : (inputs.hasOwnProperty('value') ? 'value' : null);

                // 排除引用链接的情况
                if (textKey && !Array.isArray(inputs[textKey])) {
                    const currentVal = String(inputs[textKey]).toLowerCase();
                    // 关键词匹配
                    const isPos = title.includes('positive') || title.includes('正向') || title.includes('正面') || currentVal.includes('positive');
                    const isNeg = title.includes('negative') || title.includes('负面') || title.includes('反向') || currentVal.includes('negative');

                    if (isPos) {
                        promptCandidates.pos.push({ id, idStr, key: textKey, val: '正向提示词' });
                    } else if (isNeg) {
                        promptCandidates.neg.push({ id, idStr, key: textKey, val: '负面提示词' });
                    }
                }
            }
            // WeiLinPromptToString 特殊处理
            if (type === 'WeiLinPromptToString') {
                promptCandidates.pos.push({ id, idStr, key: 'positive', val: '正向提示词' });
                promptCandidates.neg.push({ id, idStr, key: 'negative', val: '负面提示词' });
            }
        }

        // --- 筛选最佳提示词节点 ---
        // 规则：按 ID 倒序排列，取 ID 最大的一个进行替换
        if (promptCandidates.pos.length > 0) {
            promptCandidates.pos.sort((a, b) => b.id - a.id);
            const winner = promptCandidates.pos[0];
            changes.push({ id: winner.idStr, key: winner.key, val: winner.val });
        }
        if (promptCandidates.neg.length > 0) {
            promptCandidates.neg.sort((a, b) => b.id - a.id);
            const winner = promptCandidates.neg[0];
            changes.push({ id: winner.idStr, key: winner.key, val: winner.val });
        }

        // --- 执行修改 ---
        if (changes.length === 0) {
            alert('⚠️ 未识别到可自动填充的参数，或所有目标参数均为节点引用。');
            return;
        }

        // 应用所有更改
        changes.forEach(c => {
            wf[c.id].inputs[c.key] = c.val;
        });

        // 统计与报告
        const reportMap = {};
        changes.forEach(c => {
            reportMap[c.val] = (reportMap[c.val] || 0) + 1;
        });

        let reportMsg = `✅ 自动填充完成，共修改 ${changes.length} 处。\n\n`;
        // 显示具体的修改统计
        for (const [key, count] of Object.entries(reportMap)) {
            reportMsg += `• ${key}: ${count} 个\n`;
        }

        // 自检提示
        if (!promptCandidates.pos.length) reportMsg += `\n❌ 未找到【正向提示词】，请手动检查。\n`;
        if (!promptCandidates.neg.length) reportMsg += `\n❌ 未找到【负面提示词】，请手动检查。\n`;

        // 提示词多节点警告
        if (promptCandidates.pos.length > 1) reportMsg += `\nℹ️ 发现多个正向提示词节点，已自动选择 ID 最新的节点 #${promptCandidates.pos[0].idStr} 进行替换。\n`;

        if (reportMap['种子'] && reportMap['种子'] > 1) {
            reportMsg += `\nℹ️ 多个节点使用了“种子”占位符，这是正常的，它们将保持同步。\n`;
        }
        if (reportMap['采样器'] && reportMap['采样器'] > 1) {
            reportMsg += `\n⚠️ 警告：发现多个“采样器”占位符。请检查是否误将修复阶段的采样器也替换了。\n`;
        }

        // 更新 UI
        editor.value = JSON.stringify(wf, null, 2);
        editor.dispatchEvent(new Event('input')); // 触发检查器刷新
    ...
    async _updateApiPreset() {
        const imageGen = this.ctx.getModule('imageGen');
        // 核心：直接获取主模块的引用，而不是使用本地副本
        const comfySettings = imageGen.settings.comfyui;
        ...
        const presets = comfySettings.apiPresets || [];
        const index = presets.findIndex(p => p.name === this.activeApiPreset);

        if (index === -1) {
            this.ctx.helpers.showToast('未在配置中找到当前预设，请尝试重新选择', 'error');
            return;
        }

        // 1. 获取最新 UI 值
        const currentUrl = this.containerEl.querySelector('#comfyui-url')?.value.trim();
        const currentApiMode = this.containerEl.querySelector('#comfyui-api-mode')?.value;
        ...
        comfySettings.apiPresets = presets; // 确保数组引用被更新
        ...
        try {
            ...
            this.ctx.helpers.showToast(`预设 "${this.activeApiPreset}" 已更新并同步到文件`, 'success');
        } catch (e) {
            console.error(e);
    ...
    async _deleteApiPreset() {
        ...
        // 注意这里是 `imageGen.settings.comfyui`
        if (imageGen.settings.comfyui) {
            imageGen.settings.comfyui.apiPresets = this.apiPresets;
        }
        ...
        try {
            ...
            console.error('[ComfyUISettings] 删除预设后保存设置时出错：', e);
        }
        // --- 修改结束 ---
    ...
    collectSettings() {
        ...
        return {
            ...
            i2iWorkflow: this.containerEl.querySelector('#comfyui-i2i-workflow-select')?.value || '', // 
