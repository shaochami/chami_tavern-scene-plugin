...
export class SDSettings {
                ...
                <div class="tsp-settings-group" style="padding: 12px; background: var(--tsp-bg-tertiary); border-radius:8px; margin-bottom:15px;">
                    ...
                    <div class="tsp-form-row">
                        ...
                        <div class="tsp-btn-group" style="flex:1; align-self: flex-end; justify-content: flex-end;">
                            <!--  更新按钮 -->
                            <button type="button" class="tsp-btn tsp-btn-primary" id="sd-api-preset-update" title="覆盖更新当前预设">
                ...
                <div class="tsp-form-group">
                    ...
                    <div class="tsp-form-group" style="margin-top: 10px;">
                        ...
                        <div class="tsp-input-group">
                            ...
                            <button class="tsp-btn tsp-btn-icon" id="sd-auth-toggle" title="显示/隐藏密码">
                    ...
                    <div class="tsp-form-group" style="margin-top: 10px;">
                        ...
                        <p class="tsp-text-muted" style="margin-top: 5px; font-size: 0.85em;">
                            <i class="fa-solid fa-info-circle"></i> 跨域模式需要 WebUI 启动参数 <code>--cors-allow-origins "*"</code>。<br>
    ...
    bindEvents(containerEl) {
        ...
        containerEl.querySelector('#sd-auth-toggle')?.addEventListener('click', (e) => {
            e.preventDefault();
            const input = containerEl.querySelector('#sd-auth');
        ...
        containerEl.querySelector('#sd-api-preset-select')?.addEventListener('change', (e) => {
            ...
            if (this.ctx.getModule('settingsPanel')) {
                this.ctx.getModule('settingsPanel').isApiPresetModified.sd = false;
            }
        });
        containerEl.querySelector('#sd-api-preset-update')?.addEventListener('click', async () => {
        ...
        // 防傻操作：监听API预设相关输入框变化
        const urlInput = containerEl.querySelector('#sd-url');
        const authInput = containerEl.querySelector('#sd-auth');
        const apiModeSelect = containerEl.querySelector('#sd-api-mode');
        if (urlInput) {
            urlInput.addEventListener('input', () => {
                if (this.ctx.getModule('settingsPanel')) {
                    this.ctx.getModule('settingsPanel').isApiPresetModified.sd = true;
                }
            });
        }
        if (authInput) {
    ...
    async _updateApiPreset() {
        const imageGen = this.ctx.getModule('imageGen');
        const sdSettings = imageGen.settings.sd;
        ...
        const presets = sdSettings.apiPresets || [];
        const index = presets.findIndex(p => p.name === this.activeApiPreset);

        if (index === -1) {
            this.ctx.helpers.showToast('预设索引错误', 'error');
            return;
        }

        // 1. 获取 UI 值
        const url = this.containerEl.querySelector('#sd-url')?.value.trim();
        const auth = this.containerEl.querySelector('#sd-auth')?.value.trim();
        const apiMode = this.containerEl.querySelector('#sd-api-mode')?.value;
    ...
    async _deleteApiPreset() {
        ...
        // 这是关键一步，确保 imageGen 持有的设置是最新的
        if (imageGen.settings.sd) {
            imageGen.settings.sd.apiPresets = this.apiPresets;
        }

        // 重新设置当前激活的预设（例如，回到第一个或无）
        this.activeApiPreset = this.apiPresets.length > 0 ? this.apiPresets[0].name : '';

        // --- 新增修改：立即保存设置以持久化删除操作 ---
        try {
            // 调用 imageGen 的保存方法，它会处理后续的服务器文件写入
            await imageGen.saveSettings();
            this.ctx.helpers.showToast('API 预设已删除', 'success');
        } catch (e) {
            this.ctx.helpers.showToast('保存设置失败，请在关闭时手动保存', 'error');
            console.error('[SDSettings] 删除预设后保存设置时出错：', e);
        }
        // --- 修改结束 ---
